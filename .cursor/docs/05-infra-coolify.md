...
- **Redis**: host kontejner `redis-core`, mreža `redis-net` (external). User `ephelpdesk`, scoped ACL (nije `+@all`).
...
| backend | FQDN api | proxy + redis-net | `prisma migrate deploy` pa API+Socket.IO :10001 |
| worker | nema | proxy (internal) + redis-net | `node dist/src/worker.js` |
