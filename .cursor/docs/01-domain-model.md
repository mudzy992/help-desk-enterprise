...
6. **SLA** — `BusinessHoursCalendar` (`weeklyHours` JSON by ISO weekday 1–7, IANA `timezone`, holidays, `isActive`), `CalendarHoliday`, `SlaProfile` (linked calendar, `description`, `isActive`), `SlaRule` (`service` + `OU` + `priority` match keys, `evaluationOrder` then specificity for deterministic pick, first-response / resolution minutes in business time), `TicketSlaState`, `SlaEscalationRule`. Admin CRUD writes `ChangeLog` (`business_hours_calendar` / `sla_profile` / `sla_rule`). Ticket timers remain a later Phase 6 item.
