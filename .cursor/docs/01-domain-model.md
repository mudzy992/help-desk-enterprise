...
3. **Catalog & routing** — `ServiceCategory`, `Service` (`id` + unique `slug` are identity; `lifecycle` is catalog publish state only; `availability` is admin-set runtime status, overlaid by `ServiceDowntimeWindow` `[startsAt, endsAt)` and never blocks ticket creation), `FormVersion` (`id` is the persisted `formVersionRef`; schema JSON is immutable once the version is ACTIVE/RETIRED or referenced by a ticket), `RoutingRule` (`originUnit + service → group`, `isFallback`), `PriorityMatrixRule`, `ChangeLog`
