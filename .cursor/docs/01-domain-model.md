...
6. **SLA** — `BusinessHoursCalendar` (`weeklyHours` JSON by ISO weekday 1–7, IANA `timezone`, holidays, `isActive`), `CalendarHoliday`, `SlaProfile` (linked calendar, `description`, `isActive`), `SlaRule` (`service` + `OU` + `priority` match keys, `evaluationOrder` then specificity for deterministic pick, first-response / resolution minutes in business time), `TicketSlaState` (response/resolution clocks, pause while `WAITING_FOR_USER` / `PENDING_APPROVAL`, cumulative `pausedBusinessMinutes`), `SlaEscalationRule`. Admin CRUD writes `ChangeLog` (`business_hours_calendar` / `sla_profile` / `sla_rule`). Overdue escalations remain a later Phase 6 item.
7. **Ops** — `Notification`, `AuditLog` (hash chain), `IntegrationJob`, `ConfigVersion`, `AppSetting`, `GuardrailClaim` (unique `(kind, subjectKey, fingerprint)` for anti-loop/anti-spam idempotency; not a second audit engine)
