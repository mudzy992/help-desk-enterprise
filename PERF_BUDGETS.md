...
| DB upita po HTTP zahtjevu | cilj ≤ 8 prosječno (kapija 32) | **izmjereno (CI smoke 2026-09-24)**: prosjek 21,2 preko 1.827 zahtjeva; `/tickets/inbox` 28,1 · `/tickets` 25,4 · `/search` 15,4 · `/notifications/unread-count` 2,0 ✅ | `db_queries_per_request=` iz `api.log` (F0 middleware, `backend/src/common/request-context/request-metrics.middleware.ts`); run [35985764662](https://github.com/mudzy992/help-desk-enterprise/actions/runs/35985764662), komentar na PR #2. **Kapija u CI-ju je 32** = izmjereno 21,2 × 1,5 (pravilo „prag iz mjerenja"); cilj ≤ 8 je radni zadatak, a ne tvrdnja. Nalaz: budžet „≤ 2" iz F1 odnosio se na *podatkovni* upit liste (paginacija + count), ne na cijeli zahtjev — stvarni trošak čine i per-request kontekst (sesija, authz) i batch loaderi po stranici (`to-ticket-client-responses`, `load-ticket-display-labels`, SLA snapshot, close codes, CSAT). Smanjenje: prvo izmjeriti koji loaderi se pozivaju na `status=RESOLVED` stranicama, pa spojiti u jedan upit / preskočiti kad polja nisu tražena |
