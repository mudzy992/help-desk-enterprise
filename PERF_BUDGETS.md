...
| DB upita po HTTP zahtjevu | cilj ≤ 8 prosječno (kapija 12) | **izmjereno (CI smoke 2026-09-24, 120 seedanih tiketa)**: 18,4 prije `perf-07` → **8,7** (9,6 u prvom runu s podacima) (lista 12,1 · inbox 12,0 · detalj 14,0 · poruka 9,9 · `/search` 6,0 · dashboard 1,1 · unread 1,0) ⚠️ cilj otvoren | `db_queries_per_request=` iz `api.log` (F0 middleware, `backend/src/common/request-context/request-metrics.middleware.ts`); run [35985764662](https://github.com/mudzy992/help-desk-enterprise/actions/runs/35985764662), komentar na PR #2. **Kapija u CI-ju je 32** = izmjereno 21,2 × 1,5 (pravilo „prag iz mjerenja"); cilj ≤ 8 je radni zadatak, a ne tvrdnja. Nalaz: budžet „≤ 2" iz F1 odnosio se na *podatkovni* upit liste (paginacija + count), ne na cijeli zahtjev. Najveći dio ostatka bili su settingsi: 45 `*-configuration.loader.ts` fajlova čita svoje ključeve kroz `SettingsService`, jedan `await` (i jedan `findUnique`) po ključu — `perf-07` to svodi na jedno listanje tabele po zahtjevu (`settings-snapshot.ts`). Ostalo su batch loaderi po stranici (`to-ticket-client-responses`, `load-ticket-display-labels` 5 upita, SLA snapshot, close codes, CSAT) i sesija/authz (keširano u Redisu). **Pouka iz mjerenja:** isti kod je na *praznoj* bazi davao 3,6 — `loadTicketDisplayLabels`, `loadParentTicketSummaries`, `loadTicketCsatSubmissions` i `loadCloseCodesByIds` preskaču upit kad nemaju šta razriješiti. Zato smoke od sada seeda 120 tiketa: prag koji se mjeri na praznoj aplikaciji ne mjeri ništa. Smanjenje: prvo izmjeriti koji loaderi se pozivaju na `status=RESOLVED` stranicama, pa spojiti u jedan upit / preskočiti kad polja nisu tražena |
