...
export const ticketErrorMessages: Record<TicketsErrorCode, string> = {
  ...
  TIME_LOG_IMMUTABLE: 'A running timer cannot be edited; stop it first',
  ACTIVE_TIMER_ELSEWHERE: 'A timer is already running on another ticket',
  TIME_TRACKING_NOT_ALLOWED_IN_STATUS:
    'Time cannot be tracked on a resolved, closed, archived or merged ticket',
  TIME_TRACKING_UNAVAILABLE: 'Time tracking configuration is unavailable',
  TIME_LOG_OVERLAP: 'The entry overlaps another time entry of the same person',
  TIME_LOG_INVALID_RANGE: 'The time range is not valid',
  TIME_LOG_ENDED_AT_INVALID: 'The reported end of activity is not valid',
  TIME_LOG_REASON_REQUIRED: 'A reason (3-500 characters) is required',
  TIME_LOG_NOTE_REQUIRED: 'A note (3-500 characters) is required',
  TIME_LOG_EDIT_WINDOW_EXPIRED: 'The entry is older than the allowed correction window',
  MANUAL_TIME_DISABLED: 'Manual time entries are disabled',
