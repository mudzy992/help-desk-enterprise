export type TicketsErrorCode =
  | 'NOT_FOUND'
  | 'ORIGIN_UNIT_NOT_FOUND'
  | 'ORIGIN_UNIT_REQUIRED'
  | 'SERVICE_NOT_FOUND'
  | 'SERVICE_REQUIRED'
  | 'SERVICE_NOT_OFFERED'
  | 'FORM_VERSION_REQUIRED'
  | 'FORM_VERSION_NOT_FOUND'
  | 'FORM_VERSION_SERVICE_MISMATCH'
  | 'FORM_VERSION_NOT_ACTIVE'
  | 'INVALID_STATUS_TRANSITION'
  | 'STATUS_CHANGE_FORBIDDEN'
  | 'FORBIDDEN'
  | 'INVALID_TITLE'
  | 'INVALID_DESCRIPTION'
  | 'REQUESTER_NOT_FOUND'
  | 'ROUTING_UNAVAILABLE'
  | 'ASSIGNMENT_UNAVAILABLE'
  | 'GROUP_INBOX_DISABLED'
  | 'TICKET_NOT_CLAIMABLE'
  | 'PARTICIPANTS_DISABLED'
  | 'PARTICIPANT_NOT_FOUND'
  | 'PARTICIPANT_DUPLICATE'
  | 'INVALID_PARTICIPANT_ROLE'
  | 'PARTICIPANT_IDENTITY_REQUIRED'
  | 'PARTICIPANT_USER_NOT_FOUND'
  | 'PARTICIPANT_GROUP_NOT_FOUND'
  | 'PARTICIPANT_LOCKED'
  | 'CHAT_MESSAGE_TYPES_DISABLED'
  | 'INVALID_MESSAGE_TYPE'
  | 'INVALID_MESSAGE_BODY'
  | 'MESSAGE_TYPE_NOT_ALLOWED'
  | 'TIME_LOG_NOT_FOUND'
  | 'OVERLAPPING_TIMER'
  | 'TIME_LOG_NOT_ACTIVE'
  | 'TIME_LOG_IMMUTABLE'
  | 'ACTIVE_TIMER_ELSEWHERE'
  | 'TIME_TRACKING_NOT_ALLOWED_IN_STATUS'
  | 'TIME_TRACKING_UNAVAILABLE'
  | 'TIME_LOG_OVERLAP'
  | 'TIME_LOG_INVALID_RANGE'
  | 'TIME_LOG_ENDED_AT_INVALID'
  | 'TIME_LOG_REASON_REQUIRED'
  | 'TIME_LOG_NOTE_REQUIRED'
  | 'TIME_LOG_EDIT_WINDOW_EXPIRED'
  | 'MANUAL_TIME_DISABLED'
  | 'ATTACHMENTS_DISABLED'
  | 'ATTACHMENT_NOT_FOUND'
  | 'ATTACHMENT_REQUIRED'
  | 'ATTACHMENT_TOO_LARGE'
  | 'ATTACHMENT_TYPE_NOT_ALLOWED'
  | 'ATTACHMENT_FILENAME_INVALID'
  | 'ATTACHMENT_LIMIT_REACHED'
  | 'CLASSIFICATION_DOWNGRADE'
  | 'ATTACHMENTS_STORAGE_UNAVAILABLE'
  | 'ATTACHMENT_INFECTED'
  | 'ATTACHMENT_SCAN_UNAVAILABLE'
  | 'APPROVALS_DISABLED'
  | 'APPROVALS_UNAVAILABLE'
  | 'APPROVAL_NOT_FOUND'
  | 'APPROVAL_NOT_PENDING'
  | 'APPROVAL_DECISION_REQUIRED'
  | 'APPROVAL_TRANSITION_FORBIDDEN'
  | 'APPROVAL_SELF_FORBIDDEN'
  | 'APPROVAL_COMMENT_REQUIRED'
  | 'INVALID_APPROVAL_COMMENT'
  | 'REOPEN_DISABLED'
  | 'REOPEN_REQUIRED'
  | 'REOPEN_NOT_ELIGIBLE'
  | 'INVALID_REOPEN_COMMENT'
  | 'WAITING_FOR_USER_UNAVAILABLE'
  | 'REOPEN_UNAVAILABLE'
  | 'SPLIT_DISABLED'
  | 'SPLIT_UNAVAILABLE'
  | 'SPLIT_REQUIRED'
  | 'SPLIT_NOT_ALLOWED'
  | 'INVALID_SPLIT_REASON'
  | 'INVALID_SPLIT_CHILDREN'
  | 'BULK_DISABLED'
  | 'BULK_UNAVAILABLE'
  | 'BULK_ACTION_NOT_ALLOWED'
  | 'BULK_CLOSE_FORBIDDEN'
  | 'BULK_SCOPE_MISMATCH'
  | 'BULK_REASON_REQUIRED'
  | 'BULK_PREVIEW_REQUIRED'
  | 'BULK_RATE_LIMITED'
  | 'BULK_BROADCAST_INVALID'
  | 'SAVED_VIEWS_DISABLED'
  | 'SAVED_VIEWS_UNAVAILABLE'
  | 'SAVED_VIEW_NOT_FOUND'
  | 'SAVED_VIEW_LIMIT'
  | 'SAVED_VIEW_NAME_TAKEN'
  | 'INVALID_SAVED_VIEW'
  | 'HANDLER_GROUP_NOT_FOUND'
  | 'CLOSE_CODES_UNAVAILABLE'
  | 'CLOSE_CODE_INVALID'
  | 'INVALID_RESOLUTION_NOTE'
  | 'REQUIRED_FIELDS_MISSING'
  | 'REQUIRED_FIELDS_UNAVAILABLE'
  | 'REDACTION_BLOCKED'
  | 'REDACTION_UNAVAILABLE'
  | 'CONFIDENTIAL_ACCESS_DENIED'
  | 'CONFIDENTIAL_UNAVAILABLE'
  | 'BREAK_GLASS_DISABLED'
  | 'BREAK_GLASS_REASON_REQUIRED'
  | 'BREAK_GLASS_NOT_APPLICABLE'
  | 'INVALID_BREAK_GLASS_REASON'
  | 'DUPLICATE_TICKET_BLOCKED'
  | 'GUARDRAILS_UNAVAILABLE'
  | 'BULK_BROADCAST_CONFIRMATION_REQUIRED'
  | 'CSAT_DISABLED'
  | 'CSAT_UNAVAILABLE'
  | 'CSAT_NOT_ELIGIBLE'
  | 'CSAT_ALREADY_SUBMITTED'
  | 'INVALID_CSAT_RATING'
  | 'INVALID_CSAT_COMMENT'
  | 'ARCHIVE_UNAVAILABLE'
  | 'TICKET_ARCHIVED_READ_ONLY'
  | 'REMOTE_DISABLED'
  | 'REMOTE_RATE_LIMITED'
  | 'EXPORT_TOO_LARGE'
  | 'FORWARDING_UNAVAILABLE'
  | 'FORWARD_NOT_ALLOWED_IN_STATUS'
  | 'FORWARD_SAME_GROUP'
  | 'FORWARD_CROSS_OU_DISABLED'
  | 'FORWARD_CROSS_OU_FORBIDDEN'
  | 'FORWARD_REASON_REQUIRED'
  | 'FORWARD_TARGET_USER_NOT_MEMBER'
  | 'FORWARD_NO_PREVIOUS_GROUP'
  | 'PRIORITY_UNCHANGED'
  | 'PRIORITY_REQUIRED'
  | 'PRIORITY_REASON_REQUIRED'
  | 'PRIORITY_NOT_OVERRIDDEN'
  | 'TICKET_NOT_EDITABLE'
  | 'TICKET_MERGED'
  | 'TICKET_NOT_MERGED'
  | 'MERGE_SELF'
  | 'MERGE_PARENT_INVALID'
  | 'MERGE_CHILD_INVALID'
  | 'MERGE_CONFIDENTIAL_MISMATCH'
  | 'MERGE_LIMIT_EXCEEDED'
  | 'MERGE_REASON_REQUIRED';

export class TicketsError extends Error {
  constructor(
    readonly code: TicketsErrorCode,
    message = code,
    readonly details?: Readonly<Record<string, unknown>>,
  ) {
    super(message);
    this.name = 'TicketsError';
  }
}
